...
public class WndBag extends WndTabbed {
    ...
    public void item(final Item item) {
      ...
      if (item != null) {
        ...
        if (lastBag.owner.isAlive() && item.isUpgradable() && item.levelKnown) {
          ...
          int nBars = (int) GameMath.gate(0,
              Math.round(((float) NBARS * item.durability()) / item.maxDurability()), NBARS);
  ...
  private static class Placeholder extends Item {
    ...
    public Placeholder(final int image) {
      this.image = image;
    }
  ...
  public static WndBag seedPouch(final Listener listener, final Mode mode, final String title) {
    SeedPouch pouch = Dungeon.hero.belongings.getItem(SeedPouch.class);
    return pouch != null ? new WndBag(pouch, listener, mode, title)
  ...
  public WndBag(final Bag bag, final Listener listener, final Mode mode, final String title) {
    ...
    BitmapText txtTitle =
        PixelScene.createText(title != null ? title : Utils.capitalize(bag.name()), 9);
  ...
  @Override
  protected void onClick(final Tab tab) {
    hide();
  ...
  protected void placeItems(final Bag container) {
