...
public class TrapsPainter extends Painter {
    ...
    public static void paint(final Level level, final Room room) {
        ...
        Painter.fill(level, room, 1, Random.element(traps));

        Room.Door door = room.entrance();
        door.set(Room.Door.Type.REGULAR);

        int lastRow = level.map[room.left + 1 + ((room.top + 1) * Level.WIDTH)] == Terrain.CHASM ? Terrain.CHASM
                : Terrain.EMPTY;
        ...
        if (Random.Int(3) == 0) {
            ...
            level.drop(TrapsPainter.prize(level), pos).type = Heap.Type.CHEST;
        } else {
