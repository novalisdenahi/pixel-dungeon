...
public class PassagePainter extends Painter {
  ...
  private static Point p2xy(final Room room, final int p) {
    if (p < pasWidth) {
  ...
  public static void paint(final Level level, final Room room) {
    ...
    for (Point door : room.connected.values()) {
      joints.add(PassagePainter.xy2p(room, door));
    }
    ...
    do {
      Painter.set(level, PassagePainter.p2xy(room, p), floor);
      p = (p + 1) % perimeter;
  ...
  private static int xy2p(final Room room, final Point xy) {
    if (xy.y == room.top) {
